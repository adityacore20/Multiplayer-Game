...
int main() {
  ...
  Paddle rightPaddle = {SCREEN_WIDTH - 35.0f, SCREEN_HEIGHT / 2.0f - 60, 25,
                        120, 6};
  Ball ball;
  ball.radius = 20;
  ...
  while (!WindowShouldClose()) {
    ...
    if (!connected) {
      ...
      if (connectTimer > 1.0f) {
        ...
        inet_pton(AF_INET, SERVER_IP,
                  &serv_addr.sin_addr); // Use Localhost for now
    ...
    if (connected) {
      ...
      int sent = send(sock, &rightPaddle.y, sizeof(float),
    ...
    if (!connected) {
      DrawText("Attempting to connect...", SCREEN_WIDTH / 2 - 150,
               SCREEN_HEIGHT / 2, 20, RED);
    } else {
      DrawLine(SCREEN_WIDTH / 2, 0, SCREEN_WIDTH / 2, SCREEN_HEIGHT, WHITE);
      ball.Draw();
      leftPaddle.Draw();
      rightPaddle.Draw();
      DrawText(TextFormat("%d", leftScore), SCREEN_WIDTH / 2 - 100, 40, 60,
               WHITE);

      DrawText(TextFormat("%d", rightScore), SCREEN_WIDTH / 2 + 60, 40, 60,
               WHITE);

      if (gameOver) {

        if (winner == 1)
          DrawText("LEFT PLAYER WINS!", SCREEN_WIDTH / 2 - 250,
                   SCREEN_HEIGHT / 2, 50, GREEN);

        if (winner == 2)
          DrawText("RIGHT PLAYER WINS!", SCREEN_WIDTH / 2 - 250,
                   SCREEN_HEIGHT / 2, 50, GREEN);

        DrawText("Waiting for server restart...", SCREEN_WIDTH / 2 - 230,
                 SCREEN_HEIGHT / 2 + 80, 30, GRAY);
      }
    }
